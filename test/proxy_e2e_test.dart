...
void main() {
  ...
  test('真实模型名请求原样转发到上游（不做虚拟改写）', () async {
  ...
  test('模型名原样透传：虚拟 ID 条目不影响纯转发', () async {
    mock = await MockUpstream.bind();
    // 模型库中存在一个名字本身就是虚拟 ID 的模型（如上游是另一个中转站），
    // 但代理保持纯转发：请求体模型名原样透传，不做任何改写。
    ...
    expect(resp.statusCode, 200);
    // 纯转发：上游收到的就是请求体里的模型名（不改写）
    expect(mock.received.first['model'], 'chat-premium');
    await p.stop();
    for (final k in keyManager.getAll()) {
      await keyManager.deleteKey(k.id);
    }
