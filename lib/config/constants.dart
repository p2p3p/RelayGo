...
class Constants {
  ...
  static const String rateLimitEventsBox = 'rate_limit_events'; // 限流切换 Key 事件
  ...
  static const int defaultIpRateLimitPerMinute = 50; // 默认 50，0 = 不限制
  ...
  /// 自适应 TPM 学习值的下限保护：乘性减后不得低于此值，
  /// 防止学习值被压到接近 0 导致 key 永远无法通过限流检查（卡死）。
  static const int minLearnedTpm = 1000;

  /// 新建 Key 的默认 RPM（每分钟最大请求数）。
  /// 设为 0 表示不限制。之前的默认 60 在高频场景下太低，容易误触限流。
  static const int defaultMaxRpm = 0;
  ...
  // —— 限流切换 Key 事件日志 ——
  static const int rateLimitEventCap = 500; // 内存与存储保留的事件条数上限
}
