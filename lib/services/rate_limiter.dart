...
class RateLimiter {
  ...
  int? _effectiveTpmLimit(ApiKey key, String model) {
    ...
    if (adaptiveTpmEnabled) {
      ...
      if (learned != null && learned > 0) {
        // 下限保护：不得低于 minLearnedTpm，防止学习值卡死导致 key 永远被限流
        final safe = learned < Constants.minLearnedTpm
            ? Constants.minLearnedTpm
            : learned;
        // 留出余量，避免同一窗口再撞限；乘性减已在下调时埋入，这里再兜底
        return (safe * 0.95).floor();
      }
    }
    return null;
  }
  ...
  void recordUpstreamTpmLimit(ApiKey key, String model, int usageThisWindow) {
    ...
    var learned = current == null || reduced < current ? reduced : (current * 0.85).round();
    // 下限保护：不得低于 minLearnedTpm，防止学习值被压到接近 0 导致卡死
    if (learned < Constants.minLearnedTpm) learned = Constants.minLearnedTpm;
    _learnedTpm[keyed] = learned;
  ...
  /// 当前 TPM 窗口剩余可用 token 数（账户配置上限或自适应学习上限 - 已用）。
  ///
  /// 返回 -1 表示该 key 未配置 TPM 限制（不受限）。
  int tpmRemaining(ApiKey key, {String model = ''}) {
    final eff = _effectiveTpmLimit(key, model);
    if (eff == null || eff <= 0) return -1;
    final used = _keyTokenWindows[key.id]?.sum() ?? 0;
    final remaining = eff - used;
    return remaining < 0 ? 0 : remaining;
  }

  /// 当前 RPM 令牌桶剩余可突发请求数（供实时监控展示）。
  ///
  ...
  /// 该 key 当前生效的 TPM 上限（配置硬拦或自适应学习值），null 表示不限。
  int? effectiveTpmLimit(ApiKey key, {String model = ''}) =>
      _effectiveTpmLimit(key, model);

  /// 从 429 学到的每 key+model TPM 上限（自适应 AIMD 学习结果）。
  ///
  /// 返回 null 表示尚未学到该 key+model 的上限。
  int? learnedTpm(ApiKey key, {String model = ''}) =>
      _learnedTpm['${key.id}|$model'];

  /// 该 key 所有已学到的 TPM 上限条目（key+model → 上限），供监控页展示。
  Map<String, int> learnedTpmAll(ApiKey key) {
    final prefix = '${key.id}|';
    final result = <String, int>{};
    _learnedTpm.forEach((k, v) {
