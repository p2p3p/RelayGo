...
class LogService {
  ...
  /// 今天已写入的文件日期标签（用于跨日切换文件名）
  String? _todayFileDate;
  ...
  void add(RequestLog log) {
    ...
    _scheduleFlush();
    // 每天一个日志文件（JSONL 格式，每行一条）
    _appendToDailyFile(log);
  }

  /// 追加到当天日志文件（同步追加+立即落盘，确保写入即可立即可读）
  void _appendToDailyFile(RequestLog log) {
    ...
    try {
      ...
      file.writeAsStringSync(
        '${const JsonEncoder().convert(log.toJson())}\n',
  ...
  /// 关闭当天文件句柄（同步写入方案下无持久句柄，此处仅为保持语义与 dispose 调用）
  void _closeTodayFile() {
    _todayFileDate = null;
  }

  /// 当天日志文件的日期标签（如 "2025-08-27"），null = 文件日志未启用
  String? get todayFileDate => _todayFileDate;

  /// 列出已落盘的按天日志文件（按文件名倒序，最新在前）。
  ///
  /// 返回 [(文件名, 大小字节)]；目录未配置或为空时返回空列表。
  List<(String, int)> listDailyFiles() {
    final dir = logDirectory;
    ...
    try {
      for (final f in dir.listSync()) {
        ...
        files.add((f.uri.pathSegments.last, f.lengthSync()));
      }
    } catch (_) {}
  ...
  /// 读取指定日志文件全部内容（次日文件往往较大，一次性读取便于导出/查看）。
  Future<String> readDailyFile(String fileName) async {
    final dir = logDirectory;
    if (dir == null) return '';
    try {
