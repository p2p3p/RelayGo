...
/// OAuth Token 刷新回调类型
typedef EnsureFreshTokenFn = Future<bool> Function(ApiKey key);
...
class KeyManager {
  ...
  /// OAuth Token 刷新回调，由外部注入（避免循环依赖）。
  /// 在 testKey / fetchModels 前调用，确保 OAuth token 不过期。
  EnsureFreshTokenFn? ensureFreshTokenFn;
  ...
  Future<ApiKey> createKey({
    ...
    int priority = 100,
    int weight = 1,
    int maxRpm = Constants.defaultMaxRpm,
    int dailyQuota = 1000000,
    Map<String, dynamic> metadata = const {},
    Map<String, String> customHeaders = const {},
    String authType = 'api_key',
    Map<String, dynamic> oauthMetadata = const {},
  }) async {
  ...
  Future<KeyTestOutcome> _runTest(ApiKey key, Duration timeout) async {
    ...
    try {
      // ── OAuth token 刷新（与 proxy forward 一致）──
      // 测试前检查 OAuth token 是否过期，若过期则自动刷新，
      // 避免用过期 token 测试导致 401/403 误判。
      // 但刷新失败不应阻断测试：现有 token 可能仍然有效（只是即将过期），
      // 应继续用现有 token 尝试测试，让上游返回的真实状态来决定结果。
      if (key.isOAuth && ensureFreshTokenFn != null) {
        try {
          await ensureFreshTokenFn!(key);
        } catch (e) {
          // 刷新异常不阻断测试，继续用现有 token
        }
      }
  ...
  /// daily_quota / metadata 字段，可与 [exportKeys] 导出的 JSON 完整往返。
  /// metadata 字段为 JSON 字符串，用于保存 OAuth 认证文件中的 refresh_token
  /// 等附加信息。
  Future<int> importKeys(List<Map<String, String>> rows) async {
    ...
    for (final row in rows) {
      ...
      // 解析 metadata（OAuth 认证文件导入时携带）
      Map<String, dynamic> metadata = const {};
      final metaStr = row['metadata'];
      if (metaStr != null && metaStr.isNotEmpty) {
        try {
          ...
          // metadata 非合法 JSON，忽略
        }
      }

      // 解析 auth_type 和 oauth_metadata（P2 新增）
      final authType = row['auth_type'] ?? 'api_key';
      Map<String, dynamic> oauthMetadata = const {};
      if (authType == 'oauth') {
        final oauthStr = row['oauth_metadata'];
        if (oauthStr != null && oauthStr.isNotEmpty) {
          try {
            final decoded = jsonDecode(oauthStr);
            if (decoded is Map) {
              oauthMetadata = Map<String, dynamic>.from(decoded);
            }
          } catch (_) {
            // oauth_metadata 非合法 JSON，忽略
          }
        }
      }
