...
abstract class BaseProvider {
  ...
  /// 全局默认请求头（由上层注入），应用到所有提供商
  Map<String, String> get globalHeaders;
  set globalHeaders(Map<String, String> value);
}
...
abstract class BaseHttpProvider implements BaseProvider {
  ...
  /// 全局默认请求头（需求 2.2.8），由上层注入，应用到所有提供商；
  /// key 级 customHeaders 在 [forward] 中最后写入，优先级最高。
  @override
  ...
  /// 不从客户端转发到上游的请求头
  /// 按请求头名称（小写）判断是否已被设置
  static bool _hasUserAgent(Map<String, String> headers) {
    return headers.keys.any((k) => k.toLowerCase() == 'user-agent');
  }
  ...
  Future<ProviderResult> forward(ProxyRequest request, ApiKey key,
    ...
    // 全局默认请求头（需求 2.2.8）：覆盖鉴权与提供商头
    globalHeaders.forEach(upstream.headers.set);
    // key 级自定义请求头：最后写入，优先级最高（用于伪装 X-Client-ID / User-Agent 等）
    key.customHeaders.forEach(upstream.headers.set);
    // 若用户未在 customHeaders 中指定 user-agent，则使用默认值
    if (!key.customHeaders.containsKey('user-agent') &&
        !globalHeaders.containsKey('user-agent') &&
        !_hasUserAgent(request.headers)) {
